//directiva para autocompletar texto
...
.directive('ngAutocomplete', ['$timeout', '$filter', 'ngAutocompleteService', function($timeout, $filter, ngAutocompleteService) {
    ...
    return {
            ...
            scope.getResults = function () {
                ...
                delay(function() {
                    ngAutocompleteService.getData(scope.url, scope.keyword).then(function(resp) {
                        scope.results = [];
                        var filtered = $filter('filter')(resp.data.data, {title: scope.keyword});
        ...
        template:
        '<input type="text"  class="form-control" ng-model="keyword" class="ng-autocomplete-input" placeholder="{{ placeholder }}" ng-change="getResults()" ng-keyup="keyupHandler($event)" ng-blur="blurHandler()" ng-focus="currentIndex = 0" autocorrect="off" autocomplete="off">' +
